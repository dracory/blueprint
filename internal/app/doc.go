// package app provides the central dependency app for Dracory.
//
// In Dracory, the app is the single source of truth for all core
...
//
// The app is **not** a full application container or harness:
...
//
// Instead, it functions as a **typed dependency app**:
//   - All services and stores are registered at initialization.
//   - Other components fetch dependencies from the app via typed getters.
...
// Usage example:
//
//    app := NewRegistry(config)
//
//    db := app.GetDatabase()
//    userStore := app.GetUserStore()  // may be nil if disabled by config
//
package app
