...
func (controller *categoryManagerController) renderPage(r *http.Request) string {
	...
	initScript := hb.Script(`
		const urlCategoriesLoad = '` + shared.NewLinks().CategoryManager(map[string]string{"action": "load-categories"}) + `';
		const urlCategoryCreate = '` + shared.NewLinks().CategoryManager(map[string]string{"action": "create-category"}) + `';
		const urlCategoryUpdate = '` + shared.NewLinks().CategoryManager(map[string]string{"action": "update-category", "category_id": "CATEGORY_ID_PLACEHOLDER"}) + `';
		const urlCategoryDelete = '` + shared.NewLinks().CategoryManager(map[string]string{"action": "delete-category"}) + `';
		const urlCategoriesReorder = '` + shared.NewLinks().CategoryManager(map[string]string{"action": "reorder-categories"}) + `';
	`)
