...
func (controller *tagManagerController) renderPage(r *http.Request) string {
	...
	initScript := hb.Script(`
		const urlTagsLoad = '` + shared.NewLinks().TagManager(map[string]string{"action": "load-tags"}) + `';
		const urlTagPostsLoad = '` + shared.NewLinks().TagManager(map[string]string{"action": "load-tag-posts", "tag_id": "TAG_ID_PLACEHOLDER"}) + `';
		const urlTagCreate = '` + shared.NewLinks().TagManager(map[string]string{"action": "create-tag"}) + `';
		const urlTagUpdate = '` + shared.NewLinks().TagManager(map[string]string{"action": "update-tag", "tag_id": "TAG_ID_PLACEHOLDER"}) + `';
		const urlTagDelete = '` + shared.NewLinks().TagManager(map[string]string{"action": "delete-tag"}) + `';
	`)
