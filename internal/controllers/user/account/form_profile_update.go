...
func NewFormProfileUpdate(app app.AppInterface) liveflux.ComponentInterface {
...
func (c *formProfileUpdate) handleUpdate(ctx context.Context, action string, data url.Values) error {
	...
	if !c.app.GetConfig().GetUserStoreVaultEnabled() {
		...
		if err := c.app.GetVaultStore().TokenUpdate(ctx, user.GetFirstName(), c.FormFirstName, c.app.GetConfig().GetVaultStoreKey()); err != nil {
			c.app.GetLogger().Error("Error saving first name", "error", err.Error())
			c.FormError = "Saving profile failed. Please try again later."
			c.FormSuccess = ""
			return nil
		}
		if err := c.app.GetVaultStore().TokenUpdate(ctx, user.GetLastName(), c.FormLastName, c.app.GetConfig().GetVaultStoreKey()); err != nil {
			c.app.GetLogger().Error("Error saving last name", "error", err.Error())
			c.FormError = "Saving profile failed. Please try again later."
			c.FormSuccess = ""
			return nil
		}
		if err := c.app.GetVaultStore().TokenUpdate(ctx, user.GetBusinessName(), c.FormBusinessName, c.app.GetConfig().GetVaultStoreKey()); err != nil {
			c.app.GetLogger().Error("Error saving business name", "error", err.Error())
			c.FormError = "Saving profile failed. Please try again later."
			c.FormSuccess = ""
			return nil
		}
		if err := c.app.GetVaultStore().TokenUpdate(ctx, user.GetPhone(), c.FormPhone, c.app.GetConfig().GetVaultStoreKey()); err != nil {
