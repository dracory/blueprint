...
func (f *fakeBlogStore) PostList(ctx context.Context, options blogstore.PostQueryOptions) ([]blogstore.PostInterface, error) {
	...
	return f.StoreInterface.PostList(ctx, options)
}

func (f *fakeBlogStore) PostCount(ctx context.Context, options blogstore.PostQueryOptions) (int64, error) {
	if f.postCountError != nil {
		return 0, f.postCountError
	}
	return f.StoreInterface.PostCount(ctx, options)
}
