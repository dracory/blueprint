...
func (widget *blogPostListWidget) prepareData(r *http.Request) (data blogPostListWidgetData, errorMessage string) {
	conf, err := config.GetConfig(r.Context())
	if err != nil {
	...
	postList, errList := conf.BlogStore.PostList(options)

	if errList != nil {
		conf.LogStore.ErrorWithContext("Error. At blogController.page", errList.Error())
		return data, "Sorry, there was an error loading the posts. Please try again later."
	}

	postCount, errCount := conf.BlogStore.PostCount(options)

	if errCount != nil {
		conf.LogStore.ErrorWithContext("Error. At blogController.page", errCount.Error())
