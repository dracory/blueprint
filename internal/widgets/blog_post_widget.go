...
func (w *blogPostWidget) Render(r *http.Request, content string, params map[string]string) string {
	...
	if len(uriParts) < 5 {
		cfg.LogStore.ErrorWithContext("At blogPostWidget: URI mismatch", map[string]any{"uri": r.RequestURI})
	...
	if postID == "" {
		cfg.LogStore.ErrorWithContext("anyPost: post ID is missing", map[string]any{"uri": r.RequestURI})
		url := helpers.ToFlashWarningURL("The post you are looking for no longer exists. Redirecting to the blog location...", blogsUrl, 5)
		return hb.Script(`window.location.href = "` + url + `"`).ToHTML()

	}

	post, errPost := cfg.BlogStore.PostFindByID(postID)

	if errPost != nil {
		cfg.LogStore.ErrorWithContext("Error. At BlogPostController.AnyIndex. Post not found", errPost.Error())
		url := helpers.ToFlashWarningURL("The post you are looking for no longer exists. Redirecting to the blog location...", blogsUrl, 5)
		return hb.Script(`window.location.href = "` + url + `"`).ToHTML()
	}

	if post == nil {
		cfg.LogStore.ErrorWithContext("ERROR: anyPost: post with ID "+postID+" is missing", map[string]any{"postID": postID})
		url := helpers.ToFlashWarningURL("The post you are looking for no longer exists. Redirecting to the blog location...", blogsUrl, 5)
		return hb.Script(`window.location.href = "` + url + `"`).ToHTML()
	}

	if post.IsUnpublished() {
		cfg.LogStore.WarnWithContext("WARNING: anyPost: post with ID "+postID+" is unpublished", map[string]any{"postID": postID})
