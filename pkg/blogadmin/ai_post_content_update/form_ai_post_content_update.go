...
func (c *formAiPostContentUpdate) Render(ctx context.Context) hb.TagInterface {
	breadcrumbs := layouts.Breadcrumbs([]layouts.Breadcrumb{
		...
		{Name: "Post Manager", URL: shared.NewLinks("/admin/blog").PostManager()},
		{Name: "Edit Post", URL: shared.NewLinks("/admin/blog").PostUpdate(map[string]string{"post_id": c.PostID()})},
	...
	backButton := hb.A().
		...
		Href(shared.NewLinks("/admin/blog").PostUpdate(map[string]string{"post_id": c.PostID()})).
		HTML("← Back to Post")
...
func (c *formAiPostContentUpdate) onSave(ctx context.Context, successMessage string, redirect bool, data url.Values) error {
	...
	if redirect {
		c.RedirectTo = shared.NewLinks("/admin/blog").PostManager()
	} else {
		c.RedirectTo = ""
	}
