...
func NewAiToolsController(app app.AppInterface) *aiToolsController {
			...
			Child(
				hb.Div().Class("d-grid gap-3 col-8 mx-auto").
					Child(
						hb.Hyperlink().
							HTML("📝 Post Generator").
							Href(shared.NewLinks("/admin/blog").AiPostGenerator()).
							Class("btn btn-primary btn-lg fw-semibold"),
					).
					Child(
						hb.Hyperlink().
							HTML("💡 Title Generator").
							Href(shared.NewLinks("/admin/blog").AiTitleGenerator()).
					...
					Child(
						hb.Hyperlink().
							...
							Href(shared.NewLinks("/admin/blog").AiTest()),
					).
					Child(
						hb.Hyperlink().
