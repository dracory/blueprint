		...
		Children([]hb.TagInterface{
			...
			hb.Tbody().Children(lo.Map(data.blogList, func(blog blogstore.PostInterface, _ int) hb.TagInterface {
				...
				buttonAiContent := hb.Hyperlink().
					...
					Href(shared.NewLinks("/admin/blog").AiPostContentUpdate(map[string]string{"post_id": blog.GetID()})).
					Target("_blank")
				...
				buttonEdit := hb.Hyperlink().
					...
					Title("Edit").
					Href(shared.NewLinks("/admin/blog").PostUpdate(map[string]string{"post_id": blog.GetID()})).
					Target("_blank")
				...
				buttonDelete := hb.Hyperlink().
					...
					Title("Delete").
					HxGet(shared.NewLinks("/admin/blog").PostDelete(map[string]string{"post_id": blog.GetID()})).
					HxTarget("body").
					HxSwap("beforeend")
