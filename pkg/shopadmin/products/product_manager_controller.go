...
func (controller *productManagerController) page(r *http.Request, data productManagerControllerData) hb.TagInterface {
		...
		Children([]hb.TagInterface{
			...
			hb.Tbody().Children(lo.Map(data.productList, func(product shopstore.ProductInterface, _ int) hb.TagInterface {
				...
				buttonEdit := hb.Hyperlink().
					...
					Title("Edit").
					Href(shared.NewLinks("/admin/shop").ProductUpdate(map[string]string{"product_id": product.GetID()}))

				buttonDelete := hb.Hyperlink().
					Class("btn btn-danger").
					Child(hb.I().Class("bi bi-trash")).
					Title("Delete").
					HxGet(shared.NewLinks("/admin/shop").ProductDelete(map[string]string{"product_id": product.GetID()})).
					HxTarget("body").
					HxSwap("beforeend")
...
func (controller *productManagerController) sortableColumnLabel(data productManagerControllerData, columnLabel string, columnSortKey string) hb.TagInterface {
	...
	link := shared.NewLinks("/admin/shop").Products(map[string]string{
		"page":       "0",
		"by":         columnSortKey,
...
func (controller *productManagerController) tableFilter(data productManagerControllerData) hb.TagInterface {
	buttonFilter := hb.Button().
		...
		Text("Filters").
		HxPost(shared.NewLinks("/admin/shop").Products(map[string]string{
