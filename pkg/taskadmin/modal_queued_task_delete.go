...
func (a *admin) modalQueuedTaskDelete(r *http.Request, queueID string) *hb.Tag {
	if queueID == "" {
		return hb.NewDiv().Child(hb.NewSwal(hb.SwalOptions{Icon: "error", Title: "Error", Text: "queue id is required"}))
	}
