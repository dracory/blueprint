...
func (a *admin) modalTaskEnqueue(r *http.Request) *hb.Tag {
	taskList, err := a.taskStore.TaskDefinitionList(context.Background(), nil)

	if err != nil {
		a.logger.Error("At adminTasks > modalTaskEnqueue", "error", err.Error())
		return hb.NewDiv().Child(hb.NewSwal(hb.SwalOptions{Title: "Error", Text: "Error listing tasks"}))
	}
	...
	groupTasks := bs.FormGroup().
		...
		Child(bs.FormSelect().
			...
			Children(lo.Map(taskList, func(task taskstore.TaskDefinitionInterface, _ int) hb.TagInterface {
				return hb.NewOption().
					Value(task.ID()).
					Text(task.Title())
			})))
	...
	buttonEnqueue := hb.NewButton().
		...
		HxPost(link(*r, PathHome, map[string]string{
