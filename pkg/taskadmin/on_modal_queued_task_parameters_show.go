...
func (a *admin) onModalQueuedTaskParametersShow(queueID string) string {
	if queueID == "" {
	...
	if err != nil {
		a.logger.Error("At taskadmin > onModalQueuedTaskParametersShow", "error", err.Error())
	...
	return a.modalQueuedTaskParameters(queue.Parameters()).ToHTML()
}
